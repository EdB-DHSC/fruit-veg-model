...
age_max <- 5 # HARD CODE THE MAX AGE SO THAT ONLY 5 AND YOUNGER ARE INCLUDED IN THE MODEL COHORT
...
bmi_simulation <- function(age_range, input_kcal, filename = "treat.csv") {
  ...
  ages_below_5 <- c() # EMPTY VECTOR TO LIST UNDER 5 WHO ARE NEEDED
  
  if(any(age_range<5)){ # Case when we need additional 5-year olds
    
    ages_below_5 <- age_range[age_range<5] # LIST UNDER 5 NEEDED
  ...
  five_year_olds <- treatment_group %>%
    filter(initial_age == 5) # EXTRACT THE TREATMENT COHORT WITH INITIAL AGE OF 5
